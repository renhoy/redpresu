...
class ContentNote {
  ...
  processContent(content) {
    ...
    processedContent = processedContent.replace(/<p([^>]*)>/g, (match, attributes) => {
      // Si ya tiene style, añadir padding-bottom al final
      if (attributes.includes('style=')) {
        return match.replace(/style="([^"]*)"/g, 'style="$1; padding-bottom: 10px;"');
      } else {
        // Si no tiene style, añadirlo
        return `<p${attributes} style="padding-bottom: 10px;">`;
      }
    });
