...
CREATE POLICY redpresu_subscriptions_insert_own_company ON redpresu.subscriptions FOR INSERT WITH CHECK (((company_id = public.user_company_id()) AND (EXISTS ( SELECT 1
   FROM redpresu.users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['admin'::text, 'superadmin'::text])))))));
...
CREATE POLICY redpresu_subscriptions_select_own_company ON redpresu.subscriptions FOR SELECT USING (((company_id = public.user_company_id()) OR (EXISTS ( SELECT 1
   FROM redpresu.users
  WHERE ((users.id = auth.uid()) AND (users.role = 'superadmin'::text))))));
...
CREATE POLICY redpresu_subscriptions_update_own_company ON redpresu.subscriptions FOR UPDATE USING ((((company_id = public.user_company_id()) AND (EXISTS ( SELECT 1
   FROM redpresu.users
