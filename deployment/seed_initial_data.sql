...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_tables WHERE schemaname = 'redpresu' AND tablename = 'config'
  ) THEN
    RAISE EXCEPTION 'Tabla redpresu.config no existe. Ejecutar MIGRATION_TO_PRODUCTION.sql primero.';
  END IF;

  IF NOT EXISTS (
    SELECT FROM pg_tables WHERE schemaname = 'redpresu' AND tablename = 'companies'
  ) THEN
    RAISE EXCEPTION 'Tabla redpresu.companies no existe. Ejecutar MIGRATION_TO_PRODUCTION.sql primero.';
  END IF;

  IF NOT EXISTS (
    SELECT FROM pg_tables WHERE schemaname = 'redpresu' AND tablename = 'subscriptions'
  ) THEN
    RAISE EXCEPTION 'Tabla redpresu.subscriptions no existe. Ejecutar MIGRATION_TO_PRODUCTION.sql primero.';
  END IF;
...
-- NOTA: Comentado porque el nombre de la secuencia puede variar según la migración
-- Si necesitas ajustar la secuencia, ejecuta manualmente:
-- SELECT setval(pg_get_serial_sequence('redpresu.companies', 'id'), GREATEST(2, (SELECT MAX(id) FROM redpresu.companies)), false);
...
DELETE FROM redpresu.subscriptions WHERE company_id = 1;

-- Insertar nueva suscripción
INSERT INTO redpresu.subscriptions (
  company_id,
  plan,
  status,
  stripe_customer_id,
...
INSERT INTO redpresu.config (key, value, description, category, is_system, created_at, updated_at)
VALUES (
  'multiempresa',
  'true'::jsonb,
...
BEGIN
  ...
  SELECT COUNT(*) INTO config_count FROM redpresu.config;

  -- Contar empresas
  SELECT COUNT(*) INTO companies_count FROM redpresu.companies;

  -- Contar suscripciones
  SELECT COUNT(*) INTO subscriptions_count FROM redpresu.subscriptions;
...
FROM redpresu.config
ORDER BY category, key;

-- Ver empresa por defecto
SELECT * FROM redpresu.companies WHERE id = 1;

-- Ver suscripción
SELECT company_id, plan, status, current_period_end
FROM redpresu.subscriptions
WHERE company_id = 1;
...
DELETE FROM redpresu.config WHERE key IN (
  'multiempresa',
  'subscriptions_enabled',
  'subscription_plans',
  'forms_legal_notice',
...
-- DELETE FROM redpresu.companies WHERE id = 1;
